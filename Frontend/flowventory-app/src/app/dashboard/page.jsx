...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const flaggedIssues = inventory.filter(item =>
        item.status === 'pending_inspection' || (item.quantity && item.quantity < 5)
      ).length;
      ...
      const recentShipments = shipments.slice(0, 3).map(s => ({
        action: `📦 Shipment ${s.invoice_number || 'Unknown'} - ${s.item_desc || 'Items'}`,
        time: s.invoice_date ? new Date(s.invoice_date).toLocaleDateString() : 'Recent',
        type: 'shipment'
      }));
      ...
      setRecentActivity([
        { action: 'Unable to load recent activity', time: 'Just now', type: 'error' }
      ]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 rounded-3xl shadow-2xl p-8 mb-8 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-indigo-100 text-lg">Welcome back, {user?.firstName}! Here's your inventory overview.</p>
            </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 backdrop-blur-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-slate-600 mt-1 font-medium">Total Items</div>
                  </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 backdrop-blur-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-slate-600 mt-1 font-medium">Items Received This Week</div>
                  </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 backdrop-blur-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-slate-600 mt-1 font-medium">Pending Shipments</div>
                  </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 backdrop-blur-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-slate-600 mt-1 font-medium">Active Warehouse Zones</div>
                  </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 backdrop-blur-sm">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-slate-600 mb-3 font-medium">Warehouse Utilization</div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center">
                      ...
                      {dashboardData.flaggedIssues > 0 && <span className="ml-2 text-red-500 text-2xl">⚠️</span>}
                    </div>
                    <div className="text-sm text-slate-600 mt-1 font-medium">Flagged Issues</div>
                  </div>
                ...
                <div className="space-y-4">
                  {recentActivity.length === 0 ? (
                    ...
                    recentActivity.map((activity, index) => (
                      <div key={index} className="flex items-start space-x-4 p-4 bg-gradient-to-r from-slate-50 to-slate-100 rounded-2xl hover:from-slate-100 hover:to-slate-200 transition-all duration-200">
                        <div className="flex-shrink-0">
                          <div className="w-10 h-10 bg-gradient-to-r from-indigo-500 to-blue-500 rounded-xl flex items-center justify-center shadow-md">
                            <span className="text-white text-sm font-bold">
                              {activity.type === 'shipment' ? '📦' : activity.type === 'order' ? '📋' : '⚠️'}
                            </span>
                          </div>
                        </div>
